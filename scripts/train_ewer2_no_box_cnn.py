...
## call back 
mlp = models.create_mlp(train_data[data.continuous_no_box].shape[1])
...
combinedInput = concatenate([mlp.output, cnn_acoustics.output, cnn_phoneme.output])
wer = Dense(32, activation="relu")(combinedInput)
wer = Dropout(0.2) (wer)

wer = Dense(1, kernel_initializer='normal')(wer)

final_model='eWER2_no_box_cnn'

model = Model(inputs=[mlp.input, cnn_acoustics.input, cnn_phoneme.input], outputs= wer)
...
#dev
pred  = model.predict([dev_data[data.continuous_no_box], dev_mfcc, dev_feats_p]).flatten()
data.test_wer (pred, dev_data, "dev_mgb2_"+final_model)

#test
pred  = model.predict([test_data[data.continuous_no_box], test_mfcc, test_feats_p]).flatten()
