...
final_model='eWER2_black_box_dnn'
...
model.fit([train_feats_p_w_n, train_mfcc], train_data["wer"].to_numpy(),
           batch_size=32,  epochs=_EPOCHS, verbose=1)
...
#dev
pred  = model.predict([dev_feats_p_w_n, dev_mfcc]).flatten()
data.test_wer (pred, dev_data, "dev_mgb2_"+final_model)

#test
pred  = model.predict([test_feats_p_w_n, test_mfcc]).flatten()
