...
## call back 
mlp = models.create_mlp(train_data[data.continuous_glass].shape[1])
...
final_model='eWER2_glass_box_cnn'
...
#dev
pred  = model.predict([dev_data[data.continuous_glass], dev_mfcc, dev_feats_p, dev_feats_w]).flatten()
data.test_wer (pred, dev_data, "dev_mgb2_"+final_model)

#test
pred  = model.predict([test_data[data.continuous_glass], test_mfcc, test_feats_p, test_feats_w]).flatten()
