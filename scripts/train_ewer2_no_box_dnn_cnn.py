...
train_feats_p_w_n  = np.hstack((train_data[data.continuous_no_box], train_feats_p,train_feats_w))
dev_feats_p_w_n   = np.hstack((dev_data[data.continuous_no_box], dev_feats_p,dev_feats_w))
test_feats_p_w_n  = np.hstack((test_data[data.continuous_no_box], test_feats_p,test_feats_w))


## call back 
mlp = models.create_mlp(train_feats_p_w_n.shape[1])
...
final_model='eWER2_no_box_dnn_cnn'
...
model.fit([train_data[data.continuous_no_box], train_mfcc, train_feats_p, train_feats_w], train_data["wer"].to_numpy(),
           validation_data=([dev_data[data.continuous_no_box], dev_mfcc, dev_feats_p, dev_feats_w], dev_data["wer"].to_numpy()),
...
model.fit([train_feats_p_w_n, train_mfcc, train_feats_p, train_feats_w], train_data["wer"].to_numpy(),
           batch_size=32,  epochs=_EPOCHS, verbose=1)
...
#dev
pred  = model.predict([dev_feats_p_w_n, dev_mfcc, dev_feats_p, dev_feats_w]).flatten()
data.test_wer (pred, dev_data, "dev_mgb2_"+final_model)

#test
pred  = model.predict([test_feats_p_w_n, test_mfcc, test_feats_p, test_feats_w]).flatten()
